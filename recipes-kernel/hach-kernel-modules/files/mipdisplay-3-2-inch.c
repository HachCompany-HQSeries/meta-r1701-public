...
//! display. This driver exports the framebuffer character driver ex. "/dev/fb0" for user space to use.
...
static ssize_t mipdisplay_write(struct fb_info *info, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	/* TODO: only mark changed area update all for now, since Qt will mmap the fb then this api won't be called */
