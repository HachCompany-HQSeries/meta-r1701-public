...
#
# HACH - Add 'onboading' empty file to /tmp folder as an indication of RTC loss due to battary has drained completely.
#
test -x /etc/init.d/hwclock.sh && /etc/init.d/hwclock.sh start
if test -e /etc/timestamp
then
	SYSTEMDATE=`date -u +%4Y%2m%2d%2H%2M%2S`
	SYSTEMYEAR=`date -u +%4Y`
	if [ "${SYSTEMYEAR}" = "1970" ]; then
		echo "RTC year detected as ${SYSTEMYEAR}... onboarding required"
		touch /tmp/onboarding
	fi
